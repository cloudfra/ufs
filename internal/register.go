...
type Driver struct {
	...
	CreateFunc func(context.Context, string) (ufs.FS, error)
...
// GetDriver returns the driver that's registered.
func GetDriver(name string) (Driver, error) {
	return getRegistrar().get(name)
}
...
func (r *registrar) get(driverName string) (Driver, error) {
	r.RLock()
	driver, ok := r.m[driverName]
	r.RUnlock()
	if ok {
		return driver, nil
	}
	return emptyRegistration, fmt.Errorf("driver %q is not registered", driverName)
}
...
func (r *registrar) create(ctx context.Context, name string) (ufs.FS, error) {
