...
func TestFromFSInvalidPaths(t *testing.T) {
	...
	tests := []struct {
		...
		{"Open", func(fsys ufs.ReadFS, path string) error { _, err := fsys.Open(path); return err }},
		{"Stat", func(fsys ufs.ReadFS, path string) error { _, err := fsys.Stat(path); return err }},
		{"Lstat", func(fsys ufs.ReadFS, path string) error { _, err := fsys.Lstat(path); return err }},
		{"ReadFile", func(fsys ufs.ReadFS, path string) error { _, err := fsys.ReadFile(path); return err }},
		{"ReadDir", func(fsys ufs.ReadFS, path string) error { _, err := fsys.ReadDir(path); return err }},
		{"ReadLink", func(fsys ufs.ReadFS, path string) error { _, err := fsys.ReadLink(path); return err }},
	}
...
func TestFromFSEmbedFS(t *testing.T) {
	t.Parallel()
	fsys := FromFS(ufsTesting.EmbedTestFiles())
	defer ufsTesting.ValidateClose(t, fsys)()
