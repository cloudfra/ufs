...
type fsBuildMount struct {
	...
	fsys ufs.FS // non-nil for FS mounts
}
...
func (b *FSBuilder) MountFS(path string, fsys ufs.FS) *FSBuilder {
...
func (b *FSBuilder) Build(ctx context.Context) (ufs.FS, error) {
	rootName := b.name
	if rootName == "" {
...
func newBaseFS(ctx context.Context, name string) (ufs.FS, error) {
...
func applyWrappers(fsys ufs.FS, opts MountSpecOptions) (ufs.FS, error) {
	var err error
	if opts.ReadOnly {
		fsys = ReadOnly(fsys)
	}
...
func newFromMountSpec(ctx context.Context, specs []MountSpec) (ufs.FS, error) {
	var root *MountSpec
