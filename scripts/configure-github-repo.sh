...
# Applies the GitHub repository settings this repo expects, so they can be
# restored (or reproduced on a fork) with one command instead of clicking
...
# repositories, because the feature is restricted to public repos, full
# stop. See SECURITY.md for the fallback reporting path this repo uses
...
# Required reviews are deliberately left out: this repo is currently
...
# .github/workflows/ci.yaml and reviewdog.yaml - it doesn't derive from
# them automatically, so a renamed/added/removed job silently drifts out
# of sync otherwise.
# runner / dependency-review is deliberately left out: it needs GitHub
# Advanced Security and always fails without it, so requiring it would
# block every merge once this actually takes effect.
