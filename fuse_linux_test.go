...
func TestHostMountNullFS(t *testing.T) {
	...
	// Null FS reports every path as a directory.
	fi, err := os.Stat(filepath.Join(mountDir, "anything"))
	if err != nil {
		t.Fatalf("Stat: %v", err)
	}
	if !fi.IsDir() {
		t.Errorf("null FS Stat IsDir = false, want true")
	}

	// ReadDir returns an empty listing.
	entries, err := os.ReadDir(mountDir)
	if err != nil {
...
func TestHostMountNestedOverlay(t *testing.T) {
	...
	// Write to the memory root — data is retained.
	rootFile := filepath.Join(mountDir, "data.txt")
	if err := os.WriteFile(rootFile, []byte("kept"), 0o644); err != nil {
	...
	if string(data) != "kept" {
		t.Errorf("root content = %q, want %q", data, "kept")
	}

	// Null cache overlay is accessible as a directory.
