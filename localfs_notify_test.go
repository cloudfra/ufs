	...
	for w := range writers {
		...
		go func() {
			...
			for f := range filesPerWriter {
				...
				_ = osutil.WriteFile(filepath.Join(dir, name), []byte("data"))
			}
		}()
	}
	wg.Wait()
...
func TestWatchRaceRapidCreateDelete(t *testing.T) {
	...
	for i := range 30 {
		...
		if err := osutil.Remove(p); err != nil && !os.IsNotExist(err) {
			t.Error(err)
		}
	}

	// Verify the watcher is still alive and functional after the churn.
	if err := osutil.WriteFile(filepath.Join(dir, "survivor.txt"), []byte("ok")); err != nil {
		t.Fatal(err)
	}
	ec.waitFor(t, eventDeadline, func(ev notifyEvent) bool {
...
func TestWatchRaceRapidDirNesting(t *testing.T) {
	...
	for i := range 10 {
		...
		if err := osutil.WriteFile(filepath.Join(nested, "leaf.txt"), []byte("x")); err != nil {
			t.Fatal(err)
		}
	}
	...
	if err := osutil.WriteFile(filepath.Join(dir, "still_alive.txt"), []byte("ok")); err != nil {
		t.Fatal(err)
	}
	ec.waitFor(t, eventDeadline, func(ev notifyEvent) bool {
...
func TestWatchRaceDirRemoveDuringWatch(t *testing.T) {
	...
	for i := range 5 {
		if err := osutil.MkdirAll(filepath.Join(dir, fmt.Sprintf("rmdir%d", i))); err != nil {
			t.Fatal(err)
		}
	}
	...
	for i := range 5 {
		_ = osutil.RemoveAll(filepath.Join(dir, fmt.Sprintf("rmdir%d", i)))
	}

	// Watcher must still be alive — confirm by creating a new file.
	if err := osutil.WriteFile(filepath.Join(dir, "after_rmdir.txt"), []byte("ok")); err != nil {
		t.Fatal(err)
	}
	ec.waitFor(t, eventDeadline, func(ev notifyEvent) bool {
