...
# protoc-gen-go-grpc and protoc-gen-grpc-gateway output references
# google.golang.org/grpc, grpc-gateway, and friends. Every file generated by
# those two plugins gets the "rpc" build tag stamped onto it below, so that
# building this module without -tags=rpc never needs a gRPC client/server
# dependency; only the plain protobuf message types from %.pb.go are always
# built.
RPC_BUILD_TAG = { printf '//go:build rpc\n\n'; cat "$(REPOSITORY_ROOT)/$@"; } > "$(REPOSITORY_ROOT)/$@.tmp" && mv "$(REPOSITORY_ROOT)/$@.tmp" "$(REPOSITORY_ROOT)/$@"
