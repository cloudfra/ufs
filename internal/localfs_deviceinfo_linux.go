...
func linuxDeviceMapFromReader(rootPath string, r io.Reader) map[string]ufs.DeviceInfo {
...
func buildLinuxDeviceMap(rootPath string, entries []linuxMountEntry) map[string]ufs.DeviceInfo {
	result := map[string]ufs.DeviceInfo{".": defaultDeviceInfo}
...
func linuxMakeDeviceInfo(m linuxMountEntry) ufs.DeviceInfo {
	...
	return ufs.DeviceInfo{Name: name, DeviceType: dt, ThreadCount: tc}
}
...
func linuxRotational(dev string) int {
	data, err := osReadFile(filepath.Clean("/sys/block/" + dev + "/queue/rotational"))
	if err != nil {
		return -1
	}
