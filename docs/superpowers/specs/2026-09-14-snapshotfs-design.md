...
- `Create(name)` / `MkdirAll(name, perm)`: lazily create the overlay
  ...
  post-write `Stat`).
- `Remove(name)` / `RemoveAll(name)`: `Stat` `name` against the merged
  snapshot view *before* mutating anything, so `metadata` captures the node
  as it existed at removal time (for `RemoveAll` this is just the root of
  the removed subtree — descendants are not individually walked and
  logged, to keep removal of a large directory O(1) log entries rather
  than O(subtree size)); add `name` (and, for `RemoveAll`, mark it as a
  prefix tombstone) to `tombstones`, best-effort remove from the overlay if
  present (ignore `fs.ErrNotExist`), append one log entry (`operation` =
  `"remove"` / `"removeall"`). If the pre-removal `Stat` fails (e.g. `name`
  never existed), `metadata` is left absent rather than failing the whole
  removal — `RemoveAll` on a nonexistent path is already a documented
  no-op per `ufs.go`.
