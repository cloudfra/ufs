...
var (
	fakeNvmeDeviceInfo = ufs.DeviceInfo{
		Name:        "/dev/nvme0n1",
		DeviceType:  "nvme",
		ThreadCount: 2,
	}
	fakeHddDeviceInfo = ufs.DeviceInfo{
		Name:        "/dev/hdd1",
		DeviceType:  "hdd",
		ThreadCount: 1,
	}
	fakeUsbDeviceInfo = ufs.DeviceInfo{
		Name:        "/dev/usb1",
		DeviceType:  "usb",
		ThreadCount: 1,
	}
...
func TestNewDeviceInfoMap(t *testing.T) {
	t.Parallel()
	tests := []struct {
		input ufs.DeviceInfo
		want  map[string]ufs.DeviceInfo
	}{
		{
	...
	for _, tc := range tests {
		t.Run(tc.input.Name, func(t *testing.T) {
			...
			if d := cmp.Diff(tc.want, got, cmpopts.EquateComparable(ufs.DeviceInfo{})); d != "" {
...
type mountInfo struct {
	mountPath string
	incoming  map[string]ufs.DeviceInfo
}
