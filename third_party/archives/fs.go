...
func (f ArchiveFS) Open(name string) (fs.File, error) {
	...
	if f.contents != nil {
		if info, found := f.contents[name]; found {
			...
			if entries, found := f.dirs[name]; found {
				return &dirFile{info: implicitDirInfo{implicitDirEntry{path.Base(name)}}, entries: entries}, nil
			}
	...
	var fsFile fs.File
	var seenChildren map[string]bool // tracks direct children for implicit dirs
	handler := func(ctx context.Context, file FileInfo) error {
		...
		// if we see a descendant but haven't found an explicit directory
		// entry for the requested name, it must be an implicit directory
		if fsFile == nil && file.NameInArchive != name {
			fsFile = &dirFile{info: implicitDirInfo{implicitDirEntry{path.Base(name)}}}
		...
		if df, ok := fsFile.(*dirFile); ok {
			if seenChildren == nil {
				// explicit directory: use original behavior
			...
			// implicit directory: filter to direct children only
			rel := strings.TrimPrefix(file.NameInArchive, name+"/")
			childName, _, isNested := strings.Cut(rel, "/")
			if isNested {
				if !seenChildren[childName] {
					seenChildren[childName] = true
					df.entries = append(df.entries, implicitDirEntry{childName})
				}
			} else {
				seenChildren[childName] = true
				df.entries = append(df.entries, fs.FileInfoToDirEntry(file))
...
func (f ArchiveFS) Stat(name string) (fs.FileInfo, error) {
	...
	if f.contents != nil {
		...
		if _, ok := f.dirs[name]; ok {
			...
			return implicitDirInfo{implicitDirEntry: implicitDirEntry{name: path.Base(name)}}, nil
		}
	...
	handler := func(ctx context.Context, file FileInfo) error {
		...
		if fallback == nil && strings.HasPrefix(cleanName, name) {
			fallback = implicitDirInfo{implicitDirEntry{path.Base(name)}}
		}
		return nil
	}
...
func (f *ArchiveFS) ReadDir(name string) ([]fs.DirEntry, error) {
	...
	handler := func(ctx context.Context, file FileInfo) error {
		...
		if startingPath != "." {
			if _, ok := f.contents[startingPath]; !ok {
				f.contents[startingPath] = implicitDirInfo{implicitDirEntry{path.Base(startingPath)}}
			}
		}
		for dir, base := path.Dir(startingPath), path.Base(startingPath); base != "."; dir, base = path.Dir(dir), path.Base(dir) {
			...
			if _, ok := f.contents[dir]; !ok {
				f.contents[dir] = implicitDirInfo{implicitDirEntry{path.Base(dir)}}
			}
		}
