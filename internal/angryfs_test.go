...
func TestAngryFSOperations(t *testing.T) {
	dirNames := make([]string, 0, len(ufsTesting.TestassetDirList))
	for name := range ufsTesting.TestassetDirList {
	...
	tests := []struct {
		name string
		tcs  []string
		op   func(fsys ufs.FS, tc string) error
	}{
		{"Open", ufsTesting.TestassetFilenameList, func(fsys ufs.FS, tc string) error {
			_, err := fsys.Open(tc)
			return err
		}},
		{"ReadFile", ufsTesting.TestassetFilenameList, func(fsys ufs.FS, tc string) error {
			_, err := fsys.ReadFile(tc)
			return err
		}},
		{"ReadLink", ufsTesting.TestassetFilenameList, func(fsys ufs.FS, tc string) error {
			_, err := fsys.ReadLink(tc)
			return err
		}},
		{"Lstat", ufsTesting.TestassetFilenameList, func(fsys ufs.FS, tc string) error {
			_, err := fsys.Lstat(tc)
			return err
		}},
		{"ReadDir", dirNames, func(fsys ufs.FS, tc string) error {
			_, err := fsys.ReadDir(tc)
			return err
		}},
		{"MkdirAll", []string{"a", "a/b", "a/b/c", "abc", "null"}, func(fsys ufs.FS, tc string) error {
			return fsys.MkdirAll(tc, fs.ModePerm)
		}},
		{"Create", ufsTesting.TestassetCreateFileList, func(fsys ufs.FS, tc string) error {
			_, err := fsys.Create(tc)
			return err
		}},
		{"Stat", ufsTesting.TestassetFilenameList, func(fsys ufs.FS, tc string) error {
			_, err := fsys.Stat(tc)
			return err
		}},
		{"Remove", ufsTesting.TestassetFilenameList, func(fsys ufs.FS, tc string) error {
			return fsys.Remove(tc)
		}},
		{"RemoveAll", append(ufsTesting.TestassetFilenameList, dirNames...), func(fsys ufs.FS, tc string) error {
...
func mustAngryFS(tb testing.TB) ufs.FS {
