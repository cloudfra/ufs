...
// [fs.FS] contract, building a [fs.PathError], joining zero or more errors
// without unnecessary wrapping, and small string manipulations (trimming
// slashes, splitting into components, normalizing Windows separators to
// Unix ones) that every backend needs when translating between ufs's
// virtual paths and its own storage keys.
package pathutil
...
// JoinErrors returns nil if all errs are nil, returns the single non-nil
// error directly (without wrapping) if exactly one is non-nil, or
// [errors.Join] when multiple are non-nil. This avoids the join wrapper
// overhead and the change in error identity that errors.Join introduces for
