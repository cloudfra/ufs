...
func mustArchiveFS(t *testing.T) ufs.FS {
	t.Helper()
...
func mustNoDirArchiveFS(t *testing.T) ufs.FS {
	t.Helper()
...
func mustArchiveFromEntries(t *testing.T, entries ...string) ufs.FS {
	t.Helper()
...
func TestArchiveFSInvalidPaths(t *testing.T) {
	...
	tests := []struct {
		...
		{"Open", func(fsys ufs.FS, path string) error {
			_, err := fsys.Open(path)
			return err
		}},
		{"Create", func(fsys ufs.FS, path string) error {
			_, err := fsys.Create(path)
			return err
		}},
		{"MkdirAll", func(fsys ufs.FS, path string) error {
			return fsys.MkdirAll(path, fs.ModePerm)
		}},
		{"Remove", func(fsys ufs.FS, path string) error {
			return fsys.Remove(path)
		}},
		{"RemoveAll", func(fsys ufs.FS, path string) error {
			return fsys.RemoveAll(path)
		}},
		{"ReadFile", func(fsys ufs.FS, path string) error {
			_, err := fsys.(fs.ReadFileFS).ReadFile(path)
			return err
		}},
		{"ReadDir", func(fsys ufs.FS, path string) error {
			_, err := fsys.(fs.ReadDirFS).ReadDir(path)
			return err
		}},
		{"ReadLink", func(fsys ufs.FS, path string) error {
			_, err := fsys.ReadLink(path)
			return err
		}},
		{"Lstat", func(fsys ufs.FS, path string) error {
