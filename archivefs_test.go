...
func mustArchiveFS(t testing.TB) FS {
	t.Helper()
...
func testArchiveServer(t testing.TB) *httptest.Server {
	t.Helper()
...
func testDownloadAndMount(t testing.TB, ts *httptest.Server, urlPath string) FS {
	t.Helper()
	ctx := t.Context()
	client := ts.Client()
	dir := t.TempDir()
