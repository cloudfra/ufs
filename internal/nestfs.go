...
var (
	...
	_ ufs.DeviceInfoGet = (*mountMap)(nil)
)
...
func (m *mountMap) GetDeviceInfo() map[string]ufs.DeviceInfo {
...
func (fsys *nestFS) GetDeviceInfo() map[string]ufs.DeviceInfo {
...
func (fsys *nestFS) Create(name string) (ufs.File, error) {
...
func newNestFS(ctx context.Context, name string) (ufs.FS, error) {
...
func makeNestFS(ctx context.Context, fsys ufs.FS, args ...FSArgs) *nestFS {
	var a FSArgs
	if len(args) > 0 {
		a = args[0]
	}
...
func wrapFile(f fs.File, readOnly bool, mode bufferMode) (ufs.File, error) {
	if full, ok := f.(ufs.File); ok {
		return full, nil
	}
	nf := &nestFile{File: f}
...
func wrapReadOnlyFSFile(f fs.File) (ufs.File, error) {
...
func wrapFSFile(f fs.File) (ufs.File, error) {
